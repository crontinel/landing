...
**PHP timeouts.** A long-running task exceeds `max_execution_time` (typically 30s in shared hosting, 240s in Laravel Forge). The scheduler never completes, and the [`withoutOverlapping()` mutex](/blog/laravel-schedule-withoutoverlapping-explained/) stays locked, skipping the next run.
