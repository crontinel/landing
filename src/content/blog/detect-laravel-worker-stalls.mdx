...
This is a **worker stall** — the worker process is alive but not doing any work. It's worse than a crash because you get no alert, no error log, and no signal from your monitoring tools.
...
The gap between "process is running" and "jobs are being processed" is where stalls live.

## Common Causes of Worker Stalls

### 1. Redis Connection in a Degraded State
...
### 2. Memory Leak Hitting PHP's Limit
...
### 3. Deadlocked Database Transaction
...
### 4. Supervisor-managed Worker After `queue:restart`
...
## How to Detect Worker Stalls

### Method 1: Queue Depth Growth Rate
...
### Method 2: Job Completion Latency
...
### Method 3: Supervisor Status Parsing
...
### Method 4: Application-Level Heartbeat

The most reliable approach: have each worker emit a heartbeat after processing a job:
...
1. **Set a reasonable `memory_limit`** — 256M minimum for queue workers
2. **Use `queue:work --timeout=60`** — workers that exceed 60s per job are killed and restarted
3. **Monitor Redis connection health** — check `CLIENT LIST` for idle connections
4. **Add a heartbeat job** — runs every minute, logs its own completion time
5. **Use Crontinel's heartbeat monitoring** — your workers ping every N seconds, and you're alerted if the ping stops
