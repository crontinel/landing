...
A related pattern is a [Horizon config-environment mismatch](/blog/laravel-horizon-config-env-mismatch/) — where the wrong supervisor config activates after a deploy because `APP_ENV` doesn't match or the cached config is stale.
